...
int evaluate_bind_request(Slapi_PBlock* block)
{
    ...
    for (size_t index = 0; index < plugin_binding_policy->deny_rules->count; ++index)
    {
        ...
        if (has_applied_rule(&request, deny_rule) && has_met_rule_conditions(&request, deny_rule))
        {
            log_bind_request(plugin_logging_policy->denied_requests, &request);
            return 1;
        }
    }
    ...
    for (size_t index = 0; index < plugin_binding_policy->allow_rules->count; ++index)
    {
        ...
        if (has_applied_rule(&request, allow_rule) && !has_met_rule_conditions(&request, allow_rule))
        {
            log_bind_request(plugin_logging_policy->denied_requests, &request);
            return 1;
        }
    ...
    log_bind_request(plugin_logging_policy->accepted_requests, &request);
    return 0;
}
