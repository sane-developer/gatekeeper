...
binding_rule_filter_t* create_binding_rule_filter()
{
    ...
    filter->values = array_from_size(value_set_capacity);
...
binding_rule_t* create_binding_rule()
{
    ...
    rule->apply_when = array_from_size(supported_filters_count);

    rule->authenticate_when = array_from_size(supported_filters_count);
...
binding_policy_t* create_binding_policy()
{
    ...
    policy->allow_rules = array_from_size(rule_set_capacity);

    policy->deny_rules = array_from_size(rule_set_capacity);
